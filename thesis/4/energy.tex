...
Next we concern specific point on the M-ROC surface. For $c_1 = 0.075$ and $c_2  = 0.1$, the largest $P_d$ can be achieved is     $0.609$. The same $P_d$ can be also achieved when $c_1 = 0.085$ and $c_2 = 0.005$. As we can see, we have different combination of $c_1, c_2$ to achieve the same $P_d$. This provides us flexibility when designing the spectrum sensing system. Besides th    at, when $c_1 = 0.115$ and $c_2 = 0.100$, the associated $P_d$ is $0.711$. When increase $c_2$ to $0.150$, the associated $P_d     $ is still $0.711$; but when increase $c_1$ to $0.125$, the associated $P_d$ increases to $0.731$. We can conclude, when one     of $c_1$ or $c_2$ increases and other other remain the same, the associated $P_d$ may not change. Similarly, when one of $c_1$     or $c_2$ decreases and the other remains the same, the associated $P_d$ may still not change. 
...
The relationship between $V_\tau$ and $c_1$, $c_2$ is depicted in Fig. \ref{pic:2015may1a0}.   Points for  $c_1 = 0$, $c_2$ and $c_2 = 0$, $c_1$ are not part of the computation results. These points are plotted to better present the figure.  %As we can see, $V_\tau$ is a non-decreasing function with respect to $c_1$ and $c_2$.   For different $c_1$ and $c_2$, the value of $V_\tau$ could be the same. 
