...
We  assume $s_{A_i}$ $s_{B_i}$ and $n_i$ are CSCG with variance $2\sigma_{s_A}^2$, $2\sigma_{s_B}^2$  and $2\sigma_n^2$, respectively. We also assume all noise samples are independent. Another condition is all the OFDM samples whose subscripts do not belong to set $\Theta_{\tau_0}$ are independent with each other. (look at other papers who they describe this condition)
...
&+E[\tilde{n}_i\tilde{n}_j\bar{n}_k\bar{s}_l]+E[\tilde{n}_i\tilde{n}_j\bar{n}_k\bar{n}_l]+E[\tilde{n}_i\tilde{n}_j\tilde{n}_k\tilde{s}_l]+E[\tilde{n}_i\tilde{n}_j\tilde{n}_k\tilde{n}_l]
  ...
  \label{Erij}
\end{equation}
Since noise samples are governed by i.i.d Gaussian distribution with zero mean, items in \eqref{Erij} including noise samples would have zero value, e.g. $E[\bar{s}_j\bar{s}_k\bar{s}_i\bar{n}_l] = E[\bar{s}_j\bar{s}_k\bar{s}_i]E[\bar{n}_l] = 0$. Eliminate zero value items in equation \eqref{Erij}:
\begin{equation}
...
\begin{equation}
  \begin{split}
	...
	= &\left(\#\Theta_{\tau_0}(4\sigma_{s_A}^2\sigma_n^2+2\sigma_n^4) + 2(N - \#\Theta_{\tau_0})(\sigma_n^2+\sigma_{s_A}^2)^2 +  8\sum_{i\in\Theta_{\tau_0}}\tilde{a}_i^2\sigma_{s_A}^4 \right.\\
...
\begin{equation}
  \begin{split}
	f_0(R, I)= &f_0(R)f_0(I)\\
	= &\frac{1}{\sqrt{2N}\sigma_n^2\sqrt{2\pi}}\exp\left( -\frac{R^2}{4N\sigma_n^4} \right)\frac{1}{\sqrt{2N}\sigma_n^2\sqrt{2\pi}}\exp\left( -\frac{I^2}{4N\sigma_n^4} \right)\\
