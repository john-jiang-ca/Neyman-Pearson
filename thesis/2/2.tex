...
in the end, we will show for any $\mathbf{c}$, there exists non-negative $\mathbf{k}$ such that the optimal decision rule to achieve the largest $P_d$ under constraint $\mathbf{P}_f \leq \mathbf{c}$ can be written in form of 
\[
...
Let $\delta'$ denote the optimal decision rule for 
 ...
 \end{split}
 \end{equation}
then obviously $P_d(\delta') \geq P_d(\delta^3)$ (otherwise $\delta'$ cannot be the optimal decision rule for \eqref{1120t}).The solution of \eqref{1120t} can be written as $G(0.5\boldsymbol{\mu}^1_0 + 0.5\boldsymbol{\mu}^2_0)$, hence we have  
\begin{equation}
\label{1120t2}
...
\begin{equation}
0 \leq \mu_{M+1}^3 \leq  G(0.5 \bmu_0^1+0.5\bmu_0^2)\,.
\end{equation}
Substitute \eqref{1120night5} into above equation, we have 
\begin{equation}
\label{equ: 1120n}
0 \leq \mu_{M+1}^3 \leq  G(\bmu_0^3)\,.
\end{equation}
From \eqref{1120night5}, \eqref{1120night6} and \eqref{1120night7} we can see
\begin{equation}
\label{equ: 1120n1}
\bmu_0^3 \in [0, 1]
\end{equation}
...
In the following we will prove for any points $(\mu_1^0, \mu_2^0, ..., \mu_{M+1}^0)$ belong to $G'$, there exists non-negative $\mathbf{k}$ such that for any $(\mu_1, \mu_2, ..., \mu_{M+1}) \in G_s$, 
\[
\mu_{M+1} - \sum_{i=1}^{M}k_i\mu_i \leq \mu_{M+1}^0 - \sum_{i=1}^{M}k_i\mu_i^0
\]
holds. 
...
From \eqref{condition 1} and \eqref{TEMP10}, we can conclude $\mathbf{P}_f(\delta') \in \mathcal{A}_\mathbf{c} \cap \alpha^+$. Let $\mathbf{a}' = \mathbf{P}_f(\delta')$, and we have $F(\mathbf{a}') = P_d(\delta')$. This is because $\delta'$ is an ENP test with probability of false alarm is equal to $\mathbf{a}'$.  
