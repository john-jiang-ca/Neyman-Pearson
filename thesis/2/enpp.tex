...
By increasing $k_1$ and $k_2$ from $-30$ to $30$ with step $0.05$ separately, we can calculate the associated $P_{f_1}, P_{f_2}$ and $P_d$. The ROC surface is presented in Fig. \ref{fig: 2.1}. 
...
\noindent \textit{
Let $f_0$, $f_1$, ..., $f_M$ be PDFs defined on set $\mathcal{D}$ and $f_0(x) \neq 0$ holds a.e. on $\mathcal{D}$ ($i=0, 1,..., M$). For given constants $c_1, ..., c_M \in (0, 1)$, let $\mathcal{C}_\delta$ denote a set of decision rules,  such that for $\delta \in \mathcal{C}_\delta$, we have $P_{f_i} \leq c_i$.
%No. 1
...
G_s =  \{(\boldsymbol{\mu}_0, \mu_{M+1}) | \boldsymbol{\mu}_0\in [0, 1]^M, \mu_{M+1} \in [0, G(\mathbf{\bmu_0})]
    \}
\]
thus we can see $G'$ is the boundary of set $G_s$.
% TO BE ADD A FIGURE
...
To see it, assume $\mu_{M+1} > F(\bmu_0)$. Then there is a decision rule $\delta$ such that $P_{f_i}(\delta) = \mu_i$ ($i=1, ..., M$) and $P_d(\delta) > F(\bmu_0)$. This is contradictory with the definition of $F(\bmu_0)$ in \eqref{def: F}. 

The whole proof consists of the following parts: first we prove $G(\boldsymbol{\mu}_0)$ is a convex, non-decreasing function, and when $\boldsymbol{\mu}_0 \neq 0$, $G(\boldsymbol{\mu}_0) > 0$;
...
$P_d$ is not always an increasing with $P_f$ (we arrived at this conclusion in Chapter 2 section 1.1 on page 12).  This is because $\bmu_0$ is not the probability of false alarms but rather the constraints of probability of false alarm. 

Next we will show when $\boldsymbol{\mu}_0 \neq 0$, the value of $G(\boldsymbol{\mu}_0)$ is strictly larger than zero.
Let $\delta^\ast$ be the optimal decision rule for $\bmu_0$. By optimal  we mean that this decision rule provides the largest $P_d$ under the constraints $\mathbf{P}_f \leq \bmu_0$. Consider two decision rule $\delta^1$ and $\delta^2$ defined as 
\begin{equation}
  \label{2015feb09a4}
  \delta^1 \begin{cases}
...
From \eqref{2015feb09a4} and \eqref{2015feb09a5} we can see
\begin{equation}
  \begin{split}
  P_d(\delta^1) &= \int_{\emptyset}f_0(x)\mathrm{d}x\\
                &= 0
\end{split}
\end{equation}
\begin{equation}
  \begin{split}
    P_{f_i}(\delta^1) &= \int_{\emptyset}f_i(x)\mathrm{d}x\\
                &= 0
\end{split}
\end{equation}
\begin{equation}
  \begin{split}
    P_d(\delta^2) &= \int_{\mathcal{D}}f_0(x)\mathrm{d}x\\
                &= 1
\end{split}
\end{equation}
\begin{equation}
  \begin{split}
    P_{f_i}(\delta^2) &= \int_{\mathcal{D}}f_i(x)\mathrm{d}x\\
                &= 1
\end{split}
\end{equation}

Let $\mu_{min} = \min(\mu_1, \mu_2, \cdots, \mu_M)$, since $\bmu_0 \geq 0^M$, it can be seen $\mu_{min} > 0$. Construct a new randomized test $\delta^3$ where $\delta^1$ and $\delta^2$ are used with probability $1 - \mu_{min} $ and $\mu_{min}$. With decision rule $\delta^3$ we have
\begin{equation}
  P_d(\delta^3) = (1 - \mu_{min})P_d(\delta^1) + \mu_{min}P_d(\delta^2) = \mu_{min}
\end{equation}
\begin{equation}
  P_{f_i}(\delta^3) = (1 - \mu_{min})P_{f_i}(\delta^2) + \mu_{min}P_{f_i}(\delta^2) = \mu_{min} \leq \mu_i
\end{equation}
...
\]
Since $\delta^\ast $ is the optimal decision rule for $\bmu_0$, it can be seen $P_d(\delta^\ast) > P_d(\delta^3) > 0$. Since $G(\bmu_0) = P_d(\delta^\ast)$, we can conclude $G(\bmu_0) > 0$.  
...
Next we prove $\mathcal{N} \subseteq G_s$. We need to shown that if  $\forall (\mu_1, ..., \mu_{M+1}) \in \mathcal{N}$, then this point also belongs to $G_s$.
...
\begin{equation}
 \mu_{M+1} - \sum_{i=1}^{M}k_i\mu_i < \mu_{M+1}^0 - \sum_{i=0}^{M}k_i\mu_i^0\,, 
  \label{2015feb02a1}
\end{equation}
where $\mu_i = \mu_i^0$ ($i=1, 2, \cdots, M$) and $\mu_{M+1} = 0$. Since $\mu_i^0 \in (0, 1)$ from the definition of $G_s$, we can see that point $(\mu_1, ... ,\mu_M, \mu_{M+1})=(\mu_1^0, ..., \mu_M^0, 0)$ belongs to $G_s$. Hence to make all points belonging to $G_s$ lies on one side of \eqref{PI2}, we must have 
...
\label{PI3}
\end{equation}
for all the points belong to $G_s$ \cite{dantzig1951fundamental}. 

Up to now, we have proved if  $(\bmu_0^0, \mu_{M+1}^0) \in G'$ and $\bmu_0^0 \in (0, 1)^M$, there exists constants $k_1$, ..., $k_M$ such that
\begin{equation}
\mu_{M+1} - \sum_{i=1}^{M}k_i\mu_i \leq \mu_{M+1}^0 - \sum_{i=0}^{M}k_i\mu_i^0
\label{PPI3}
\end{equation}
...
\label{PPI32015}
\end{equation}
holds for  $(\mu_1, \mu_2, ...\mu_{M+1}) \in G_s$ and the constants in \eqref{PPI32015} are non-negative, we can conclude that 
for any point $(\mu_1^0, \mu_2^0, ..., \mu_{M+1}^0)$ belongs to $G_s$, there exists non-negative $\mathbf{k}$ such that 
\begin{equation}
\mu_{M+1} - \sum_{i=1}^{M}k_i\mu_i \leq \mu_{M+1}^0 - \sum_{i=1}^{M}k_i\mu_i^0
\end{equation}
holds for any $(\mu_1, \mu_2, ..., \mu_{M+1}) \in G_s$.
...
Since we have provd if $(\bmu_0^0, \mu_{M+1}^0) \in G'$ and $\bmu_0^0 \in (0, 1)^M$, there exists non-negative constants $k_1$, ..., $k_M$ such that
\begin{equation}
\mu_{M+1} - \sum_{i=1}^{M}k_i\mu_i \leq \mu_{M+1}^0 - \sum_{i=0}^{M}k_i\mu_i^0
\label{PPI32015}
\end{equation}
...
\eqref{TEMP4} can be fulfilled if and only if $\mathcal{S}^\ast$ maximize 
\begin{equation}
\int_{\mathcal{S}}f_{0}(x)\mathrm{d}x - \sum_{i=1}^{M}\int_{\mathcal{S}}k_if_i(x)\mathrm{d}x
  \label{2015feb2a3}
\end{equation}
among all $S$. To maximize \eqref{2015feb2a3}, $\mathcal{S}^\ast$ should include all the integral range when $f_{0}(x)- \sum_{i=1}^{M}k_if_{i}(x) > 0$ and exclude the integral range when $f_{0}(x)- \sum_{i=1}^{M}k_if_{i}(x) < 0$, i.e.   
...
\end{equation}
Using \cite{equ: pf and pd} and calculating the  partial derivatives results in
\begin{equation}
\label{pro: Pf par k}
