...
This suggests $G(\bmu_0)$ is a non-decreasing function of $\bmu_0$. This is not contradictory with the conclusion we reach from the example in Section (2.1.1). In that example we study the relationship between the probability of detection and the probability of false alarm, here we study the relationship between the probability of detection ($G(\bmu_0)$) and the probability of false alarm constraints ($\bmu_0$).   

Next we will show when $\boldsymbol{\mu}_0 \neq 0$, the value of $G(\boldsymbol{\mu}_0)$ is strictly larger than zero.
Let $\delta^\ast$ be the optimal decision rule for $\bmu_0$. By optimal  we mean that this decision rule provides the largest $P_d$ under the constraints $\mathbf{P}_f \leq \bmu_0$. 
Assume $P_d(\delta) = 0$, i.e.
\[
  \int_{\mathcal{S}^\ast} f_0(x) \mathrm{d}x = 0\,.
\]
This implies the measure of $\mathcal{S}^\ast$ is zero. Thus we can see
\[
  P_{f_l} = \int_{\mathcal{S}}f_l(x)\mathrm{d}x  = 0 < \mu_l\;\;\;\;(l = 1, 2, \cdots, M)
\]
Since all $P_{f_l}$ ($l = 1, 2, \cdots, M$) are strictly smaller than its associated constraints, we can enlarge the integral range ($\mathcal{S}^\ast$) to increase $P_d$ while keeping $\mathbf{P}_f \leq \bmu_0$ (This is also shown on \cite{zhang2000efficient} page 157). This is contradictory with the condition that $\delta^\ast $ is the optimal decision rule for $\bmu_0$. Hence we can conclude $P_d(\delta^\ast) > 0$, i.e. $G(\bmu_0) > 0$.  
